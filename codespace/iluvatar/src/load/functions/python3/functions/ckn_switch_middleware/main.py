...
def main(args):
    ...
    if image_b64 is not None:
        ...
        was_cold = not os.path.exists('/tmp/models/model_{}.pth'.format(model_name))
        if was_cold:
            model = load_model(model_name)
            torch.save(model, '/tmp/models/model_{}.pth'.format(model_name))
        else:
            model = torch.load('/tmp/models/model_{}.pth'.format(model_name))
        load_model_time = time()
        prediction, probability = predict(preprocessed_input,model)
        prediction_time = time()
        return {"body": {"Success! Using model":model_name, "Probability":probability, "Cold": not os.path.exists('/tmp/models/model_{}.pth'.format(model_name)),"Total Time (s)": prediction_time-start, "Pre-process Time (s)": preprocess_time-start,"Model Load Time (s)": load_model_time-preprocess_time,"Inference Time (s)":prediction_time-load_model_time} }
    else:
        return {"body": {"No imput image!"} }
...
# Prepare the args
model_list = ['googlenet','resnet152','shufflenet_v2_x0_5','densenet201','mobilenet_v3_small','resnext50_32x4d']
image_path = "/home/exouser/ckn-faas/codespace/ckn/jetsons/device/data/images/d2iedgeai3/cat.2.jpg"

result_dict = {model: {True: {}, False: {}} for model in model_list}
...
for i in range(run_times):
    ...
    result = main(args)
    # os.system('rm -rf ~/.cache/torch/hub')
    # os.system('rm -rf /tmp/models/*')
    cold_start = result["body"]["Cold"]
    total_time += float(result["body"]["Total Time (s)"])+0.4
    result_dict[selected_model][cold_start]['p-time'] = result["body"]["Pre-process Time (s)"]
    result_dict[selected_model][cold_start]['l-time'] = result["body"]["Model Load Time (s)"]
    result_dict[selected_model][cold_start]['i-time'] = result["body"]["Inference Time (s)"]
    end = time()
    ttime += end-start
    # preprocess_time = result["body"]["Pre-process Time (s)"]
    # model_load_time = result["body"]["Model Load Time (s)"]
    # inference_time = result["body"]["Inference Time (s)"]
    # print(result)
print(total_time/run_times)
print(result_dict)
print(ttime/run_times)