...
- Single-model confidence: ModelSize==1, grouped by model name (from selected_models),
  using <model>_prob when available.
- Ensemble confidence: ModelSize>1, grouped by ModelSize (ignore policy),
  using row-level confidence column (Confidence if exists else Accuracy).

Produces:
- model_wise_confidence.csv
- ensemble_size_confidence.csv
- confidence_bar.png (same style as your accuracy bar plot)
"""
...
def safe_int_series(s: pd.Series, default: int = 0) -> pd.Series:
    x = pd.to_numeric(s, errors="coerce").fillna(default).astype(int)
    return x

def plot_like_yours(model_wise: pd.DataFrame, ensemble_wise: pd.DataFrame, out_plot: Path):
    # Only show size 2 and 3 in the first two bars (like your figure)
    ens_map = {int(r["ModelSize"]): float(r["avg_confidence"]) for _, r in ensemble_wise.iterrows()}
    set2 = ens_map.get(2, float("nan"))
    set3 = ens_map.get(3, float("nan"))

    model_names = model_wise["single_model"].astype(str).tolist()
    model_vals  = model_wise["avg_confidence"].astype(float).tolist()
...
def main():
    ...
    # Row-level confidence column (your logs may store confidence in Accuracy field)
    ...
    model_wise = df_m1.groupby("single_model").agg(
        ...
        avg_confidence=("confidence_used", "mean")
