...
if __name__ == "__main__":
    ...
    with grpc.insecure_channel(server_address) as channel:
        ...
        for i in range(11):
            ...
            est_time = get_estimated_invoke_time(stub, fqdns)
            ...
            if est_time is not None:
                ...
                # print(f"{est_time:.4f}")
                service_time = service_time * 1000 / count #averaged by chunk size
                # print(f"{service_time:.4f} ms")
                if i>0: #exclude the first one
                    service_time_tt.append(service_time)
            else:
                print(f"[{datetime.now()}] Failed to get estimate.")
        # service_time_tt = service_time_tt / 10
        print(f'----{count}---')
        # print(f"{service_time_tt:.4f} ms")
        print(service_time_tt)